...
func TestDeviceTriggerFiresOnEdgeOnly(t *testing.T) {
	...
	if e.triggerFired("a1#0", tr, now.Add(-5*time.Second), now, "", map[string]bool{"s1": true}, nil, settings) {
...
func TestSensorTriggerFiresOnRisingEdge(t *testing.T) {
	...
	if e.triggerFired("a2#0", tr, now.Add(-5*time.Second), now, "", nil, map[string]float64{"temp": 20}, settings) {
		t.Fatal("sensor trigger fired below threshold")
	}
	// Crossing above: fires once.
	if !e.triggerFired("a2#0", tr, now.Add(-5*time.Second), now, "", nil, map[string]float64{"temp": 30}, settings) {
		t.Fatal("sensor trigger did not fire on crossing")
	}
	// Still above: does not re-fire.
	if e.triggerFired("a2#0", tr, now.Add(-5*time.Second), now, "", nil, map[string]float64{"temp": 31}, settings) {
...
func TestTimeTriggerMatchesMinuteOnce(t *testing.T) {
	...
	if !e.triggerFired("a3#0", tr, now.Add(-5*time.Second), now, stamp, nil, nil, settings) {
