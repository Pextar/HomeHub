...
func TestSession_RoundTrip(t *testing.T) {
	...
	id, version, expires, ok := verifySession(secret, value)
	if !ok {
	...
	if time.Until(expires) <= 0 {
		t.Errorf("expires = %v, want in the future", expires)
	}
}
...
func TestSession_RejectsTamperedSignature(t *testing.T) {
	...
	if _, _, _, ok := verifySession(secret, tampered); ok {
...
func TestSession_RejectsWrongSecret(t *testing.T) {
	...
	if _, _, _, ok := verifySession([]byte("secret-two-bbbbbbbbbbbbbbbbbbbbbb"), value); ok {
...
func TestSession_RejectsExpired(t *testing.T) {
	...
	if _, _, _, ok := verifySession(secret, value); ok {
...
func TestSession_RejectsMalformed(t *testing.T) {
	...
	for _, v := range []string{"", "nope", "a:b:c", "id:notanint:123:sig"} {
		if _, _, _, ok := verifySession(secret, v); ok {
...
func TestSession_VersionBumpInvalidatesOldCookie(t *testing.T) {
	...
	_, version, _, ok := verifySession(secret, old)
	if !ok {
