...
func assistantTestServer(t *testing.T) *Server {
	...
	return &Server{Store: st, SessionSecret: []byte("test-secret-32-bytes-long-padxxx"), Audio: testAudio(st), Announce: testAnnouncer(), Speakers: testSpeakers(st), Music: testMusic(st)}
}
