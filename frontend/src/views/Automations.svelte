<script lang="ts">
    ...
    const shown = $derived(filter === "all" ? v.automations : v.automations.filter(a => a.trigger.type === filter));
...
<Topbar title="Automations" subtitle="{enabledCount} enabled · {totalRuns} runs total">
    {#snippet actions()}
...
{:else}
    ...
    {#if shown.length === 0}
        <p class="field-help">No automations match this filter.</p>
    {:else}
