 ...
 *
 * It spans both bridges, which is why it carries a *kind* rather than a bare
 * id: a Sonos zone is started through its coordinator's queue, a KEF speaker
 * through Spotify Connect, because a KEF's own API can play and pause but has
 * nothing to be handed. The two are not interchangeable, and saying which is
 * which is the whole job of this type.
 *
 * This is also the seam the media protocol lands on. A `MediaZone` is the same
 * idea with the vendor split removed — "these speakers, playing this,
 * together" — so it becomes another kind here (or supersedes both) without any
 * surface having to learn about it.
 */
export type Dest = { kind: "sonos" | "kef"; id: string };
...
export function createDestination(sonos: SonosBridge, kef: KEFBridge): Destination {
  const s = $state({ current: null as Dest | null });

  // Unreachable KEF speakers are left out: they have no Connect device while
  // they're off the network.
  ...
  return {
    ...
    settle() {
      ...
      s.current = livePick ?? kefPick ?? options[0] ?? null;
    },
  };
}
