...
import type { SpotifyConnectView } from "../types";
...
export const connectApi = {
  ...
  spotifyConnect() {
    return req<SpotifyConnectView>("/spotify/connect");
  },
  ...
  spotifyConnectTransfer(deviceID: string, play = true) {
    return req<void>("/spotify/connect/transfer", {
      method: "PUT",
      body: json({ device_id: deviceID, play }),
    });
  },
  ...
  spotifyConnectVolume(deviceID: string, level: number) {
    return req<void>("/spotify/connect/volume", {
      method: "PUT",
      body: json({ device_id: deviceID, level }),
    });
  },
};
