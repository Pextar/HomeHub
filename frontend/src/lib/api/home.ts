/**
 * Sockets, rooms, schedules, scenes, sensors and the
 * profiles that see them — the original house, before it had speakers.
 *
...
export const homeApi = {
  ...
  createUser(body: UserCreate) {
    return req<UserCreateResponse>("/users", { method: "POST", body: json(body) });
  },
  updateUser(id: string, body: UserUpdate) {
    return req<User>(`/users/${encodeURIComponent(id)}`, { method: "PUT", body: json(body) });
  },
  deleteUser(id: string) {
    return req<void>(`/users/${encodeURIComponent(id)}`, { method: "DELETE" });
  },
  ...
  lookupInvite(token: string) {
    return req<{ username: string }>(`/invite?token=${encodeURIComponent(token)}`);
  },
  // acceptInvite sets the password and returns a session cookie in the response.
  acceptInvite(token: string, password: string) {
    return req<{ username: string }>("/invite", {
      method: "POST",
  ...
  // Sockets
  listSockets() {
    return req<Socket[]>("/sockets");
  },
  createSocket(body: Partial<Socket>) {
    return req<Socket>("/sockets", { method: "POST", body: json(body) });
  },
  learnSocket(body: { protocol?: string; code?: string } = {}) {
    return req<{ code: string; protocol: string }>("/sockets/learn", {
      method: "POST",
      body: json(body),
    });
  },
  updateSocket(id: string, body: Partial<Socket>) {
    return req<Socket>(`/sockets/${encodeURIComponent(id)}`, { method: "PUT", body: json(body) });
  },
  deleteSocket(id: string) {
    return req<void>(`/sockets/${encodeURIComponent(id)}`, { method: "DELETE" });
  },
  socketOn(id: string) {
    return req<Socket>(`/sockets/${encodeURIComponent(id)}/on`, { method: "POST" });
  },
  socketOff(id: string) {
    return req<Socket>(`/sockets/${encodeURIComponent(id)}/off`, { method: "POST" });
  },
  socketToggle(id: string) {
    return req<Socket>(`/sockets/${encodeURIComponent(id)}/toggle`, { method: "POST" });
  },
  socketToggleFavorite(id: string) {
    return req<Socket>(`/sockets/${encodeURIComponent(id)}/favorite`, { method: "POST" });
  },
  socketTimer(id: string, body: { action: SocketAction; in_seconds: number; note?: string }) {
    return req<Timer>(`/sockets/${encodeURIComponent(id)}/timer`, {
      method: "POST",
      body: json(body),
    });
  },
  allOn() {
    return req<BulkResult>("/sockets/all/on", { method: "POST" });
  },
  allOff() {
    return req<BulkResult>("/sockets/all/off", { method: "POST" });
  },

  // Rooms
  listRooms() {
    return req<RoomSummary[]>("/rooms");
  },
  createRoom(body: { name: string }) {
    return req<Room>("/rooms", { method: "POST", body: json(body) });
  },
  updateRoom(id: string, body: { name: string }) {
    return req<Room>(`/rooms/${encodeURIComponent(id)}`, { method: "PUT", body: json(body) });
  },
  deleteRoom(id: string) {
    return req<void>(`/rooms/${encodeURIComponent(id)}`, { method: "DELETE" });
  },
  roomOn(room: string) {
    return req<BulkResult>(`/rooms/${encodeURIComponent(room)}/on`, { method: "POST" });
  },
  roomOff(room: string) {
    return req<BulkResult>(`/rooms/${encodeURIComponent(room)}/off`, { method: "POST" });
  },
  ...
  createSchedule(body: Partial<Schedule>) {
    return req<Schedule>("/schedules", { method: "POST", body: json(body) });
  },
  setAllSchedules(enabled: boolean) {
    return req<{ enabled: boolean; changed: number }>(
      `/schedules/all/${enabled ? "enable" : "disable"}`,
      { method: "POST" },
    );
  },
  updateSchedule(id: string, body: Partial<Schedule>) {
    return req<Schedule>(`/schedules/${encodeURIComponent(id)}`, {
      method: "PUT",
      body: json(body),
    });
  },
  deleteSchedule(id: string) {
    return req<void>(`/schedules/${encodeURIComponent(id)}`, { method: "DELETE" });
  },

  // Automations
  listAutomations() {
    return req<Automation[]>("/automations");
  },
  createAutomation(body: Partial<Automation>) {
    return req<Automation>("/automations", { method: "POST", body: json(body) });
  },
  updateAutomation(id: string, body: Partial<Automation>) {
    return req<Automation>(`/automations/${encodeURIComponent(id)}`, {
      method: "PUT",
      body: json(body),
    });
  },
  deleteAutomation(id: string) {
    return req<void>(`/automations/${encodeURIComponent(id)}`, { method: "DELETE" });
  },
  runAutomation(id: string) {
    return req<Automation>(`/automations/${encodeURIComponent(id)}/run`, { method: "POST" });
  },
  runAutomationRule(id: string, ruleIdx: number) {
    return req<Automation>(`/automations/${encodeURIComponent(id)}/rules/${ruleIdx}/run`, {
      method: "POST",
    });
  },
  ...
  createGroup(body: Partial<Group>) {
    return req<Group>("/groups", { method: "POST", body: json(body) });
  },
  updateGroup(id: string, body: Partial<Group>) {
    return req<Group>(`/groups/${encodeURIComponent(id)}`, { method: "PUT", body: json(body) });
  },
  deleteGroup(id: string) {
    return req<void>(`/groups/${encodeURIComponent(id)}`, { method: "DELETE" });
  },
  groupAction(id: string, action: SocketAction) {
    return req<BulkResult & { group: string }>(`/groups/${encodeURIComponent(id)}/${action}`, {
      method: "POST",
    });
  },
  ...
  // Scenes
  listScenes() {
    return req<Scene[]>("/scenes");
  },
  createScene(body: Partial<Scene>) {
    return req<Scene>("/scenes", { method: "POST", body: json(body) });
  },
  updateScene(id: string, body: Partial<Scene>) {
    return req<Scene>(`/scenes/${encodeURIComponent(id)}`, { method: "PUT", body: json(body) });
  },
  deleteScene(id: string) {
    return req<void>(`/scenes/${encodeURIComponent(id)}`, { method: "DELETE" });
  },
  activateScene(id: string) {
    return req<BulkResult & { scene: string }>(`/scenes/${encodeURIComponent(id)}/activate`, {
      method: "POST",
    });
  },
  ...
  listActivity(limit = 50) {
    return req<ActivityEntry[]>(`/activity?limit=${limit}`);
  },

  // iOS Shortcuts helper — ready-made Basic auth header for the configured creds.
  shortcutAuth() {
    return req<{ header: string }>("/shortcut-auth");
  },

  // Timers
  listTimers() {
    return req<Timer[]>("/timers");
  },
  createTimer(body: {
  ...
  deleteTimer(id: string) {
    return req<void>(`/timers/${encodeURIComponent(id)}`, { method: "DELETE" });
  },

  // Sensors
  listSensors() {
    return req<Sensor[]>("/sensors");
  },
  createSensor(body: Partial<Sensor>) {
    return req<Sensor>("/sensors", { method: "POST", body: json(body) });
  },
  updateSensor(id: string, body: Partial<Sensor>) {
    return req<Sensor>(`/sensors/${encodeURIComponent(id)}`, { method: "PUT", body: json(body) });
  },
  deleteSensor(id: string) {
    return req<void>(`/sensors/${encodeURIComponent(id)}`, { method: "DELETE" });
  },
  ...
  postSensorReading(id: string, body: { value: number; time?: string }) {
    return req<SensorReading>(`/sensors/${encodeURIComponent(id)}/readings`, {
      method: "POST",
      body: json(body),
    });
  },
  ...
  updateSettings(body: Settings) {
    return req<Settings>("/settings", { method: "PUT", body: json(body) });
  },
