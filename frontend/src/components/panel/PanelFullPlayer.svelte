<script lang="ts">
    ...
    import PanelPlayerRecord from "./PanelPlayerRecord.svelte";
    ...
    // The stage's box, measured on the element below and handed to the
    // record, which is the only thing whose size depends on it.
    let stageW = $state(0);
    let stageH = $state(0);
