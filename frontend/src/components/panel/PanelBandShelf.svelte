<script lang="ts">
     ...
     *
     * Both are Sonos-only, because both lists are. A KEF or a zone gets no
     * shelf rather than an empty one (§15.1) — the player row centres in the
     * band instead, which is the shape the reference draws.
     */
    ...
    const mode = $derived(upNext.length > 0 ? "next" : favorites.length > 0 ? "fav" : "none");
</script>

{#if mode !== "none"}
    <section class="shelf" aria-label={mode === "next" ? "Up next" : "Favorites"}>
        <h3 class="s-label">{mode === "next" ? "Up next" : "Favorites"}</h3>
        <div class="s-row">
            {#if mode === "next"}
