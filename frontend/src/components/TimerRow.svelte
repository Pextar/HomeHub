<script lang="ts">
    ...
    let countdown = $state(formatCountdown(timer.fires_at));

    // Tick the live countdown once per second.
    onMount(() => {
        const id = setInterval(() => { countdown = formatCountdown(firesAt); }, 1000);
...
<div class="row">
    <span class="action" data-action={timer.action === "on" ? "on" : "off"}>{timer.action}</span>
    <div class="info">
...
<style>
    ...
    .info { min-width: 0; }
    .when { color: var(--text-faint); font-size: 12px; }
